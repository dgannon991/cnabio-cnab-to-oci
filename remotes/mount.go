...
func (w *manifestWalker) collectCopyTasks(ctx context.Context, desc ocischemav1.Descriptor, parent *descriptorProgress, ix int) ([]copyTask, error) {
	...
	for _, c := range children {
		childCopyTasks, err := w.collectCopyTasks(ctx, c, descProgress, ix+1)
		if err != nil {
			return nil, err
		}
